...
class GeneralMulticlassTrainer(Trainer):

    def __init__(self, model, train_loader, trainer_config, train_evaluator, test_evaluator, dev_evaluator=None):
        super(GeneralMulticlassTrainer, self).__init__(model, train_loader, trainer_config, train_evaluator, test_evaluator, dev_evaluator)
    ...
    def train_epoch(self, epoch):
        ...
        if self.use_tensorboard:
            self.writer.add_scalar('{}/train/kl_div_loss'.format(self.train_loader.dataset.NAME), total_loss, epoch)
    ...
    def train(self, epochs):
        ...
        for epoch in range(1, epochs + 1):
            ...
            if self.use_tensorboard:
                ...
                self.writer.add_scalar('{}/dev/pearson_r'.format(self.train_loader.dataset.NAME), dev_scores[0], epoch)
                self.writer.add_scalar('{}/dev/kl_div_loss'.format(self.train_loader.dataset.NAME), new_loss, epoch)
